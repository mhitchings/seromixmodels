...
data {
  ...
  real gammadiff_prior;//Prior mean of the difference between IgG among seronegatives and IgG among seropositives
}
...
parameters {
  ...
  real<lower=-4,upper=4> beta_covs[r_b+1]; // beta coefficients (seropositivity log-odds)
  real<lower = 0,upper=10> gamma_diff; //baseline difference in seroresponse for seropositives vs seronegatives
  real<lower=-3,upper=5> gamma_covs[r_g]; // gamma coefficients (seroresponse mean)
}
...
model {
  ...
  for (i in 1:N) {
    ...
    // Probability of seropositivity for the individual (including covariates)
    p = expit(dot_product(beta_covs,reg_vars_b[i,]));
